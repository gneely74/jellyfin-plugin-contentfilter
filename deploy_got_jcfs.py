...
def deploy(dry_run: bool = False):
    ...
    if not MEDIA_DIR.exists():
        print(f"Error: Media directory {MEDIA_DIR} does not exist or is not mounted.", file=sys.stderr)
        return False
    ...
    s01_source_file = s01e01_orig if s01e01_orig.exists() else (s01e01_curr if s01e01_curr.exists() else None)
    ...
    if s01_source_file and s01_source_file.exists():
        ...
        s01_lang_cues = [
            c for c in parsed
            if c["category"].startswith("Language.") or c["channel"] == "audio"
        ]
